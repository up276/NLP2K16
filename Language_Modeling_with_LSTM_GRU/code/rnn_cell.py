...
class BasicLSTMCell(RNNCell):
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    with vs.variable_scope(scope or type(self).__name__):  # "BasicLSTMCell"
      ...
      # first you apply forget gate update on c, and then apply input gate upader on i
      new_c = (c * sigmoid(f + self._forget_bias) + self._activation(j)) 
      #new_c = (c * sigmoid(f + self._forget_bias) + sigmoid(i) * self._activation(j)) 
      #with all gates :  (c * sigmoid(f + self._forget_bias) + sigmoid(i) * self._activation(j)) 
      #without input gate :  new_c = (c * sigmoid(f + self._forget_bias) + self._activation(j)) 
